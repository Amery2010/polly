...
const styleTemplate = (
  input: string,
  { style }: Pick<SsmlOptions, "style">
) => {
  if (!style) return input;
  return `<mstts:express-as style="${style}">${input}</mstts:express-as>`;
};

const prosodyTemplate = (
  input: string,
  { pitch, rate }: Pick<SsmlOptions, "pitch" | "rate">
) => {
  if (!pitch && !rate) return input;
  return `<prosody pitch="${Math.floor(
    (pitch || 1) * 100
  )}%" rate="${Math.floor((rate || 1) * 100)}%">${input}</prosody>`;
};
